...
public class App {
    ...
    public static Connection conectar() {
        ...
        try {
            ...
            strSqlTeste = "CREATE TABLE `teste`.`tbl_teste` (`id` INT NOT NULL AUTO_INCREMENT,`nome` VARCHAR(45) NULL,PRIMARY KEY (`id`));";//texto corrido
            stmSql = conn.createStatement();
            stmSql.addBatch(strSqlTeste);
            stmSql.executeBatch();
            System.out.println("Ok! Tabela criada.");
            stmSql.close();
            stmSql.executeQuery(strSqlTeste);
